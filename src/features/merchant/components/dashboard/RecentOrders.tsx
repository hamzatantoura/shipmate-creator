...
import { DashboardOrder, fmtSYP } from "./types";
      ...
      <CardHeader className="flex-row items-center justify-between space-y-0">
        ...
        <Button variant="ghost" size="sm" asChild className="text-xs h-7">
          <Link to="/merchant/orders" className="flex items-center gap-1">
            {t("recentOrders.viewAll")} <Arrow className="h-3.5 w-3.5" />
          </Link>
        </Button>
      </CardHeader>
      <CardContent className="p-0">
        {orders.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
            ...
            <p className="text-sm">{t("recentOrders.empty")}</p>
          </div>
        ) : (
          <div className="divide-y divide-border">
            {orders.map((o) => {
              ...
              return (
                <div key={o.id} className="flex items-center justify-between gap-3 px-4 py-3 hover:bg-muted/40 transition-colors">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className="text-[11px] text-muted-foreground">{relativeTime(o.created_at)}</span>
                    </div>
                  </div>
                  <div className="text-left shrink-0">
                    <p className="text-sm font-bold text-foreground">{fmtSYP(amount)}</p>
                    <p className="text-[11px] text-muted-foreground font-mono">#{o.id.slice(0, 6)}</p>
                  </div>
                </div>
              );
            })}
