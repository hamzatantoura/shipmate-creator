...
DECLARE
  ...
  v_masked_phone text;
BEGIN
  -- Trim + uppercase + remove whitespace for full tracking_number match
  v_input := upper(regexp_replace(coalesce(p_code, ''), '\s', '', 'g'));

  -- ATTEMPT 1: Match against shipments.tracking_number directly (e.g. SL-EE9864-SRNI)
  ...
  IF v_order.id IS NULL THEN
    -- ATTEMPT 2: legacy 6-char prefix on orders.id (strip SL- and any dashes)
    ...
    WHERE deleted_at IS NULL
      AND replace(id::text, '-', '') ILIKE v_clean || '%';
    ...
    WHERE o.deleted_at IS NULL
      AND replace(o.id::text, '-', '') ILIKE v_clean || '%'
    LIMIT 1;
  END IF;
